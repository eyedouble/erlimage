...
readFile ( ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[] ) {
    ...
    ERLIMAGE_INTERNAL_DO ReadResponse = Erlimage_Io::fileRead ( filePath );

    if ( ReadResponse.Status != 0 )
        result = error ( env, "Error reading file" );

    ERLIMAGE_INTEROP_DO ExportResponse = Erlimage_Pixels::exportPixels ( env, Dib );
...
/*  ---------------------------
    ...
    ---------------------------  */
// static ERL_NIF_TERM
// writeFile ( ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[] ) {
...
// static ERL_NIF_TERM
// crop ( ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[] ) {
